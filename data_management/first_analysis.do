...
*/
// load the dataset
use "`PATH_DATA'/gps_demo_donate_gdp_formatted"
...
/* 
gps_demo_donate_gdp_pop data
*/

clear

//load the dataset
use "`PATH_DATA'/gps_demo_donate_gdp_pop"

// plot altruism and humanitarian aid contributions per GDP

graph twoway (scatter funding_per_capita altruism, msize(small)) (lfit funding_per_capita altruism), by(year) ytitle(Humanitarian Aid Contribution per capita) xtitle(Altruism)
graph export "`PATH_FIGURES'/funding_per_capita_altruism_scatter.pdf", replace


// Plot mean funding per capita on altruism.
graph twoway ///
	(scatter avg_funding_per_capita altruism if year==2019 & ///
		avg_funding_per_capita>0.1, msize(small)) ///
	(lfit avg_funding_per_capita altruism if year==2019 & ///
		avg_funding_per_capita>0.1), ///
